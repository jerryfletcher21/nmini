...
$ nmini events-fetch <self-public-key> '[10050]' <self-relays> '{}' | nmini events-send "$(nmini events-fetch <receiver-public-key> '[10050]' <receiver-relays> '{}' | jq '[ .tags[][1] ]')" "{}"
...
# print chat history
find <dir>/<chatting-peer> -mindepth 1 -maxdepth 1 -type f | sort -n | xargs -I '{}' cat '{}' | jq -r '. | "[\(.created_at.date)] \(.pubkey.bech32[4:8]) | \(.content)"'
```
